...
│      -> VLM propõe conceitos -> SAM3 PCS localiza
│      -> RegionProposal[] com concept
│
...
│
└── MultimodalReasoner
       -> Qwen local ou Gemini Robotics ER (opt-in)
       -> SceneContext + SemanticClaim[]
```

Fakes determinísticos ficam em [`infrastructure/fakes/`](../src/visual_perception/infrastructure/fakes/).

Na configuração atual o DINOv2 recebe o frame de 640 x 480 redimensionado para maior
aresta 448 (448 x 336), com patch 14 x 14: `FeatureMap.shape = [24, 32, 768]`. Cada
posição carrega um vetor 768D de conteúdo visual aprendido, não um label humano.

## Relação entre os backends

```text
SAM3
    define onde olhar ("estes pixels pertencem à mesma região", nunca "isto é uma porta")

DINOv2
    descreve visualmente patches e regiões

Qwen / Gemini
    propõe significado a partir de pixels e contexto estruturado (não recebe o vetor DINO)

CLIP
    mede compatibilidade entre a evidência visual e os conceitos textuais propostos
```

Exemplo resumido: o reasoner propõe `wooden panel` com alternativa `wooden door`; o CLIP
compara `CLIP(masked_subject)` e `CLIP(contextual_crop)` com os dois textos e pode
divergir entre as views. A pipeline preserva essa divergência para refinamento e
calibração; o suporte do CLIP é relativo, não uma probabilidade calibrada.

## Regra arquitetural

O domínio não importa bibliotecas de SAM, DINO, CLIP, Qwen ou Gemini. Dependências
pesadas, device, quantização, carregamento de checkpoint, credenciais e transforms
específicos ficam na infraestrutura. Isso permite testar a pipeline sem GPU nem rede,
trocar modelos em benchmarks sem reescrever o domínio, comparar backends por capacidade
e registrar a proveniência do modelo em cada observação.
...
Não atualize esta página com um modelo apenas planejado. O backend deve existir no código
ou em benchmark explicitamente identificado como candidato.
